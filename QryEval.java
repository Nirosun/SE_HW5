...
public class QryEval {
  ...
  public static void main(String[] args) throws Exception {
    ...
    while((query = br.readLine()) != null) {
      ...
      //outputResults(params.get("trecEvalOutputPath"), queryID, result);
      outputResults(bw, queryID, result);
      queryID ++;
    }
    br.close();
    bw.close();
   ...
   * @throws IOException 
   */
  static void outputResults(BufferedWriter writer, int queryID, QryResult result) throws IOException {
    
	//BufferedWriter writer = null;
    //writer = new BufferedWriter(new FileWriter(new File(trecName)));
    ...
    if (s < 1) {
      ...
      ///////////////////////
      for (int i = 0; i < s; i++) {
    	writer.write(queryID + " Q0 " + getExternalDocid (result.docScores.getDocid(s-1-i))
    			+ " " + (i+1) + " " + result.docScores.getDocidScore(s-1-i)
    			+ " run-1");
    	writer.newLine();
      }
    }
